...
void update() {
	...
	if (clicked_car_index != -1) {
		...
		if (cars[clicked_car_index].x <= 0 || cars[clicked_car_index].x >= W_WIDTH - CAR_WIDTH ||
			...
			if (r == 3) {  //araba yok etme çalýþmalarý (iþe yaramama ihtimali var)
				parking(1);
				r -= 3;
			}
			else if (g == 3) {
		...
		for (int i = 0; i < noc; i++) {
			if (cars[i].que == -1) {
				SDL_Rect clicked_car_rect = { (int)cars[clicked_car_index].x, (int)cars[clicked_car_index].y, CAR_WIDTH, CAR_HEIGHT }; //çalýþýyor ama hatalý gözüküyor
